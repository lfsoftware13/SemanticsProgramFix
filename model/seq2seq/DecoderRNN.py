...
class DecoderRNN(BaseRNN):
    ...
    def forward_step(self, input_var, hidden, encoder_outputs, function, encoder_mask=None):
        ...
        if self.use_attention:
            if encoder_mask is not None:
                dim_len = len(output.shape)
                encoder_mask = encoder_mask.view(encoder_mask.shape[0], *[1 for i in range(dim_len - 2)], encoder_mask.shape[-1])
                self.attention.set_mask(encoder_mask)
    ...
    def forward(self, inputs=None, encoder_hidden=None, encoder_outputs=None,
                    function=F.log_softmax, teacher_forcing_ratio=0, encoder_mask=None):
        ret_dict = dict()
        if self.use_attention:
